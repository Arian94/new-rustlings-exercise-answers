...
mod tests {
    ...
    fn result_with_list() {
        ...
        let mut x: Vec<i32> = Vec::new(); //... Fill in here!

        for each in division_results.into_iter() {
            x.push(each.unwrap());
        }
    ...
    fn list_of_results() {
        let numbers = vec![27, 297, 38502, 81];
        let division_results = numbers.into_iter().map(|n| divide(n, 27));
        let mut x: Vec<Result<i32, DivisionError>> = Vec::new(); //... Fill in here!

        for each in division_results.into_iter() {
            x.push(Ok(each.unwrap()));
        }
